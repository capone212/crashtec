...
class AgentsMonitor(object):
    ...
    def run(self):
        ...
        while (True):
            ...
            for record in tasks:
                record = self._promote_task_progress(record)
    ...
    def _promote_task_progress(self, task_record):
        try:
            _logger.info("promoting task %s...", self.impl.get_task_id(task_record))
            ...
            if not next_agent_class:
                ...
                return
            _logger.info("next agent class for task %s is %s", self.impl.get_task_id(task_record), next_agent_class)
            # Select next agent for job
            compatible_agents = self.impl.get_compatible_agent_instances(next_agent_class, task_record)
            next_agent_record = self.impl.chose_best_agent_for_task(compatible_agents, task_record)
            if (not next_agent_record):
                # there are no ready agents for this task
                _logger.warning("Can't promote task %s: there is no compatible agents.", self.impl.get_task_id(task_record))
                return
            _logger.info("next agent instance for task %s is %s", self.impl.get_task_id(task_record), self.impl.get_agent_id(next_agent_record))
            ...
            _logger.error("Can't promote task %s: internal error occurred during promoting", self.impl.get_task_id(task_record))
...
class Implementation(object):
    ...
    def get_compatible_agent_instances(self, class_type, task_record):
        return monitordetails.get_compatible_agent_instances(class_type, task_record)
    
    def chose_best_agent_for_task(self, agents_list, task_record):
        return monitordetails.chose_best_agent_for_task(agents_list, task_record)
