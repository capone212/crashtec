...
def parse_checker_output(checker_output):
    ...
    if (not checker_output.endswith(SUCCESS_STRING)):
        raise CtGeneralError("Can't parse dump checker response. Probably invalid dump passed.")
    ...
    if (not matchObj):
        raise CtGeneralError("Can't parse dump checker response. Probably unknown output format.")
    cpu_architecture = matchObj.group(1)
    if not (cpu_architecture in _PLATFORMS_MAP):
        raise CtGeneralError("Unknown CPU architecture %s." % cpu_architecture)
